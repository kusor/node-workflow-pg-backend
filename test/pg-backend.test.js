...
var WorkflowPgBackend = require('../lib/workflow-pg-backend');
...
test('setup', function (t) {
    ...
    backend = WorkflowPgBackend(config.backend.opts);
...
test('get job info', function (t) {
    ...
    t.test('from existing job', function (t) {
        backend.getInfo(aJob.uuid, function (err, info) {
            t.ifError(err);
            t.ok(info);
...
test('teardown', function (t) {
    vasync.forEachParallel({
        ...
        t.ifError(err, 'Drop tables error');
