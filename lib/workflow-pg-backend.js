...
WorkflowPgBackend.prototype.updateJob = function (job, callback) {
  ...
  self.client.query(query, vals, function (err, result) {
    if (err) {
...
WorkflowPgBackend.prototype._tableExists = function (table_name, callback) {
  var self = this;
  return self.client.query('select count (*) from' +
    '  information_schema.tables where table_name=$1', [table_name],
    function (err, res) {
    ...
    wf_workflows: function (cbk) {
      return self._tableExists('wf_workflows', function (exists) {
        if (!exists) {
          ...
          return self.client.query(q, function (err, res) {
            if (err) {
              return cbk(err);
            } else {
          ...
          return self.client.query(q, function (err, res) {
            if (err) {
              ...
              return async.forEachSeries(idx, function (query, cb) {
                self.client.query(query, function (err, res) {
                  if (err) {
                    return cb(err);
                  } else {
                    return cb(null);
