...
var WorkflowPgBackend = module.exports = function (config) {
    ...
    function countJobs(callback) {
        ...
        var q = 'select count(uuid), execution from wf_jobs where %s group' +
            ' by execution;';

        var stats = [{
            name: 'all_time',
            query: sprintf(q, ('created_at < to_timestamp(' + yesterday + ')'))
        }, {
            name: 'past_24h',
            query: sprintf(q, ('created_at > to_timestamp(' + yesterday +
                        ') AND created_at < to_timestamp(' + _1hr + ')'))
        }, {
            name: 'past_hour',
            query: sprintf(q, ('created_at > to_timestamp(' + _2hr +
                        ') AND created_at < to_timestamp(' + _1hr + ')'))
        }, { name: 'current',
            query: sprintf(q, ('created_at > to_timestamp(' + _1hr + ')'))
        }];
        ...
        vasync.forEachParallel({
            ...
            func: function _countJobs(stat, cb) {
                stat.results = {};
                _execQuery(stat.query, function (err, res) {
                    if (err) {
                    ...
                    if (res.rowCount) {
                        res.rows.forEach(function (r) {
                            ...
                            stat.results[r.execution] = r.count;
                        });
                    }
                    return cb(null, stat);
                });
