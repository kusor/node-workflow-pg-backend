...
var WorkflowPgBackend = module.exports = function (config) {
        ...
        _execQuery(query, vals, function (err, res) {
            if (err) {
                ...
                if (typeof (job.locks) !== 'undefined') {
                    ...
                    ' VALUES ($1, $2)';
                    _execQuery(q, [job.uuid, job.locks], function (er1, res1) {
    ...
    function validateJobTarget(job, callback) {
        ...
        var query = 'SELECT COUNT(*) FROM wf_jobs WHERE ' +
            ...
            'execution=\'waiting\'';
        // FIXME: This is not considering params could be stored in different
        // order. Review what would happen if moving to hstore, otherwise
        // exclude params from this query and lookup manually later
        var vals = [job.workflow_uuid, job.target, JSON.stringify(job.params)];
        ...
        _execQuery(query, vals, function (err, res) {
            if (err) {
                ...
                if (res.rows.length !== 0 && Number(res.rows[0].count) !== 0) {
                    return callback(new wf.BackendInvalidArgumentError(
    ...
    function updateJobProperty(uuid, prop, val, callback) {
    ...
    // - callback - f(err, jobs)
    function getJobs(execution, callback) {
        var jobs = [];
        var executions = [
            'queued',
            'failed',
        ...
        return _execQuery(query, function (err, res) {
            if (err) {
                return callback(new wf.BackendInternalError(err.Error));
            } else {
                res.rows.forEach(function (row) {
    ...
    function addInfo(uuid, info, callback) {
        ...
        _execQuery(query, vals, function (err, res) {
            if (err) {
                ...
                _execQuery(q, vals, function (err2, res2) {
                    if (err2) {
                        return callback(new wf.BackendInternalError(
                                err2.Error));
                    } 
        ...
        _execQuery(query, vals, function (err, res) {
            if (err) {
                ...
                return _execQuery(q2, vals, function (err2, res2) {
                    if (err2) {
                        return callback(new wf.BackendInternalError(err2.Error));
